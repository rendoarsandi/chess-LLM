...
export function TournamentManagement() {
  ...
  const fetchData = async () => {
    setIsLoading(true)
    try {
      const [tData, pData] = await Promise.all([getTournaments(), getPlayers()])
  ...
  const handleSave = async () => {
    if (
      ...
      newTournament.participantIds.length < 2
    ) {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-muted-foreground font-medium">
              Schedule and manage Swiss System tournaments.
            </p>
          </div>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Setup a new tournament by selecting models and defining the number of rounds.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-6 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewTournament((prev) => ({ ...prev, startTime: e.target.value }))
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewTournament((prev) => ({ ...prev, timeControlSettings: e.target.value }))
                    }
                  />
                </div>
                    ...
                    {[
                      ...
                      return (
                        <div key={group.label} className="space-y-2">
                          <h3
                            className={cn(
                              'text-[10px] font-black uppercase tracking-widest px-2 border-l-2 border-current',
                              group.color,
                            )}
                          >
                            {group.label}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {groupPlayers.map((player) => (
                              <label
                                key={player.id}
                                className="flex items-center gap-3 p-2 hover:bg-muted rounded cursor-pointer transition-colors border border-transparent hover:border-border bg-background/50"
                              >
                                <input
                                  type="checkbox"
                                ...
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-[9px] text-muted-foreground font-black tracking-widest">
                                    {player.rating} ELO
                                  </span>
                                </div>
                              </label>
              ...
              {isLoading ? (
                ...
                tournaments.map((t) => (
                  <TableRow key={t.id} className="group hover:bg-muted/30 transition-colors">
                    <TableCell className="font-black uppercase italic tracking-tight text-primary text-lg">
                      {t.name}
                    </TableCell>
                    <TableCell>
                      <div
                    ...
                    <TableCell className="font-mono text-xs">
                      <div className="flex flex-col">
                        ...
                        <span className="flex items-center gap-1.5 text-muted-foreground">
                          <Clock className="w-3 h-3" /> {new Date(t.startTime).toLocaleTimeString()}
                        </span>
                      </div>
                    </TableCell>

    ...
    >
      <div
        className={cn(
          ...
          !evaluation
            ? 'border-neutral-800'
            : 'border-neutral-700 shadow-[0_0_10px_rgba(0,0,0,0.5)]',
        )}
      >
        {/* White portion */}
        <div
        ...
        >
          {isHorizontal ? (
            <>
              <span className={percentage <= 50 ? 'text-white' : 'text-transparent'}>
                {percentage <= 50 ? formatScore() : ''}
              </span>
              <span className={percentage > 50 ? 'text-black' : 'text-transparent'}>
                {percentage > 50 ? formatScore() : ''}
              </span>
            </>
          ) : (
      ...
      {!isHorizontal && variations && variations.length > 1 && (
        <div className="absolute left-full ml-4 top-0 bg-neutral-900/90 backdrop-blur-md border border-white/10 p-3 rounded-lg opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none z-50 w-48 shadow-2xl">
          <div className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2 border-b border-white/5 pb-1">
            Top Lines
          </div>
              ...
              <div key={i} className="flex flex-col gap-0.5">
                <div className="flex justify-between items-center text-[10px] font-mono">
                  ...
                  <span
                    className={cn(
                      'font-bold',
                      (v.score || 0) >= 0 ? 'text-emerald-400' : 'text-red-400',
                    )}
                  >

...
export function ThinkingPanel({
  ...
  const truncatedReasoning =
    reasoning && reasoning.length > 150 ? reasoning.slice(0, 150) + '...' : reasoning
  ...
  return (
    ...
    >
      <div
        className={cn(
          'p-3 border-b border-border flex justify-between items-center',
          isWhite ? 'bg-primary/5' : 'bg-muted/50',
        )}
      >
        ...
        <span
          className="text-[10px] font-bold text-foreground/70 truncate max-w-[100px]"
          title={modelName}
        >
          {modelName}
        </span>
                ...
                <motion.div initial={{ x: -10 }} animate={{ x: 0 }} transition={{ delay: 0.1 }}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {candidates.map((move, i) => (
                      <div
                        key={i}
                        className="bg-muted px-2 py-1 rounded text-xs font-mono font-black border border-border/50"
                      >

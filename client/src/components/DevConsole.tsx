...
export const DevConsole: React.FC = () => {
  ...
  const handleDbAction = async (endpoint: string, method: string = 'GET', body?: object) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/admin/db/${endpoint}`,
        {
          method,
      ...
      if (data.success) {
        if (endpoint === 'execute') addLog('result', JSON.stringify(data.result, null, 2))
          ...
          addLog(
            'info',
            `Available Tables: ${data.tables.map((t: TableInfo) => t.name).join(', ')}`,
          )
      } else {
  ...
  if (!isOpen && errorCount === 0) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 z-[10000] bg-slate-800 text-white p-2 rounded-full shadow-lg hover:bg-slate-700 transition-colors pointer-events-auto"
  ...
  if (!isOpen) {
    return (
      <button
        onClick={() => setIsOpen(true)}
        className="fixed bottom-4 right-4 z-[10000] bg-red-600 text-white p-2 rounded-full shadow-lg hover:bg-red-700 animate-pulse transition-colors flex items-center gap-2 pointer-events-auto"
  ...
  return (
    <div
      className={cn(
        ...
        isMinimized
          ? 'bottom-4 w-64 h-12'
          : 'bottom-4 w-[450px] h-[600px] max-w-[95vw] max-h-[90vh]',
      )}
    >
      <div
      ...
      {!isMinimized && (
        <>
          ...
          >
            {logs.length === 0 ? (
              <div className="text-slate-500 italic p-4 text-center">No logs.</div>
            ) : (
              logs.map((log) => (
                <div
                  key={log.id}
                    ...
                    log.type === 'error'
                      ...
                      : log.type === 'warn'
                        ...
                        : log.type === 'info'
                          ...
                          : log.type === 'sql'
                            ? 'bg-purple-950/30 border-purple-500 text-purple-200 font-bold'
                            : log.type === 'result'
                              ? 'bg-green-950/30 border-green-500 text-green-200 whitespace-pre-wrap'
                ...
                >
                  ...
                  {log.type === 'sql' && (
                    <span className="mr-1 text-purple-400 italic">SQL&gt;</span>
                  )}
                  {log.message}
                  <button
          ...
          <form
            ...
            className="p-2 bg-slate-800 border-t border-slate-700 flex gap-2"
          >
            <input
              type="text"
              value={sqlInput}
              ...
              className="flex-1 bg-black/40 border border-slate-600 rounded px-2 py-1 text-[11px] font-mono focus:outline-none"

...
export function Leaderboard({ players, onSelectPlayer, loading }: LeaderboardProps) {
  ...
  const sortedPlayers = [...players].sort((a, b) => {
    const rA = variant === 'chess960' ? (a.rating960 ?? 1200) : a.rating
    const rB = variant === 'chess960' ? (b.rating960 ?? 1200) : b.rating
    return rB - rA
  })
            ...
            <AnimatePresence initial={false}>
              {sortedPlayers.map((player, index) => (
                <motion.tr
                  ...
                  layout
                  initial={{ opacity: 0, x: variant === 'standard' ? -10 : 10 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: variant === 'standard' ? 10 : -10 }}
                ...
                >
                  ...
                  <TableCell className="text-right py-4 px-4">
                    <span className="font-black text-primary text-sm md:text-lg tracking-tighter italic">
                      {variant === 'chess960' ? (player.rating960 ?? 1200) : player.rating}
                    </span>
                  </TableCell>

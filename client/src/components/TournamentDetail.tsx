...
export function TournamentDetail() {
  ...
  if (!tournament)
    return (
      <div className="flex-1 flex items-center justify-center font-black uppercase tracking-widest text-destructive">
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Button
          ...
          className="font-black tracking-widest gap-2 uppercase italic text-[10px]"
        >
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex flex-wrap gap-6 text-xs font-bold text-muted-foreground uppercase tracking-widest">
              <div className="flex items-center gap-2">
                <Users className="w-4 h-4" /> {participants.length} Models
              </div>
              <div className="flex items-center gap-2">
                <Swords className="w-4 h-4" /> Round {tournament.currentRound} of{' '}
        ...
        <div className="bg-card rounded-2xl border border-border p-6 shadow-xl min-h-[400px]">
          <div className="flex gap-8 border-b border-border mb-8 overflow-x-auto no-scrollbar">
            <h3 className="text-xl font-black uppercase italic tracking-widest text-primary pb-4 border-b-2 border-primary">
              Tournament Arena
            </h3>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                {games
                  .filter(
                    (g) =>
                      ...
                      g.roundNumber === tournament.currentRound,
                  )
                  .map((game) => (
                    <PairingRow
                      key={game.id}
                      ...
                      onClick={() => navigate(`/arena/${game.id}`)}
                    />
                  ))}
                {games.filter(
                  (g) =>
              ...
              {tournament.currentRound > 1 && (
                <div className="pt-8 space-y-6">
                  <h4 className="text-sm font-black uppercase tracking-widest px-2 text-muted-foreground">
                    Previous Rounds
                  </h4>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent border-border/50">
                      <TableHead className="font-black uppercase tracking-widest text-[9px] w-12">
                        Pos
                      </TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[9px]">
                        Model
                      </TableHead>
                      <TableHead className="text-right font-black uppercase tracking-widest text-[9px]">
                        Pts
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {participants.map((p, idx) => (
                      <TableRow key={p.id} className="hover:bg-primary/5 border-border/20 group">
                        <TableCell className="font-black italic text-muted-foreground group-hover:text-primary transition-colors">
                          #{idx + 1}
                        </TableCell>
                        <TableCell>
    ...
    >
      ...
      <div className="flex items-center gap-4 min-w-[100px] justify-end relative z-10">
        {!isLive ? (
          ...
          <Button
            size="sm"
            className="h-8 font-black tracking-widest text-[9px] gap-2 animate-pulse"
          >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5 font-black text-4xl italic">
        VS
      </div>
    </div>
  )

...
const PlayerProfile = lazy(() =>
  import('@/components/PlayerProfile').then((m) => ({ default: m.PlayerProfile })),
)
const GameHistory = lazy(() =>
  import('@/components/GameHistory').then((m) => ({ default: m.GameHistory })),
)
const AnalysisMode = lazy(() =>
  import('@/components/AnalysisMode').then((m) => ({ default: m.AnalysisMode })),
)
const AdminSettings = lazy(() =>
  import('@/components/AdminSettings').then((m) => ({ default: m.AdminSettings })),
)
const TournamentManagement = lazy(() =>
  import('@/components/TournamentManagement').then((m) => ({ default: m.TournamentManagement })),
)
const TournamentList = lazy(() =>
  import('@/components/TournamentList').then((m) => ({ default: m.TournamentList })),
)
const TournamentDetail = lazy(() =>
  import('@/components/TournamentDetail').then((m) => ({ default: m.TournamentDetail })),
)
                ...
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Comparative performance metrics across all integrated LLM architectures.
                    </p>
                  </div>
                  <div className="bg-card rounded-xl border border-border p-6 shadow-xl">
                    <Leaderboard
                      players={state.leaderboard}
                      onSelectPlayer={(id) => navigate(`/profiles/${id}`)}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Select a model to view detailed performance metrics and history.
                    </p>
                  </div>
                  {[
                    {
                      ...
                      color: 'text-primary',
                      provider: 'gemini',
                      description: 'Models from the Google Gemini family.',
                    },
                    {
                      label: 'Groq Arena',
                    ...
                    },
                    {
                      ...
                      color: 'text-blue-500',
                      provider: 'system',
                      description: 'Classical engines and built-in bots.',
                    },
                  ].map((group) => {
                    const groupPlayers = state.players.filter(
                      (p) => p.type === 'llm' && p.provider === group.provider,
                    )
                    if (groupPlayers.length === 0) return null
                    return (
                      <div key={group.label} className="space-y-6">
                        <div
                          className={cn('space-y-1 border-l-4 border-current pl-6', group.color)}
                        >
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Review past encounters and analyze patterns.
                    </p>
                  </div>
                  <div className="bg-card rounded-xl border border-border p-4 md:p-8 shadow-xl">
                    <GameHistory
                      ...
                      onSelect={(game) => state.handleSelectGame(game)}
                      onDelete={state.handleDeleteGame}
                      onClearAll={state.handleClearHistory}
                    />
                  </div>
                </div>
              </div>

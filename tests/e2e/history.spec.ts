...
test.describe('ChessLLM History', () => {
  test('should load history and view a past game', async ({ page }) => {
    ...
    if (count === 0) {
      ...
      await page.locator('button').filter({ hasText: /MATCH/ }).filter({ visible: true }).first().click();
      await expect(page.locator('div').filter({ hasText: /^ongoing$/ }).filter({ visible: true }).first()).toBeVisible({ timeout: 15000 });
  ...
  test('should navigate through moves in history mode', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const moveList = page.locator('div').filter({ hasText: /^Moves$/ }).first();
      await expect(moveList).toBeVisible({ timeout: 15000 });

      const moveButtons = page.locator('button').filter({ hasText: /^[a-h][1-8]$|^[NBRQK][a-h][1-8]$|^O-O|^O-O-O/ }).filter({ visible: true });

...
test.describe('ChessLLM Arena', () => {
  ...
  test('should start a new match and see moves appearing', async ({ page }) => {
    ...
    // Click Start Match
    const startButton = page.locator('button').filter({ hasText: /LAUNCH MATCH|START NEW MATCH/ }).filter({ visible: true }).first();
    await expect(startButton).toBeEnabled();
    await startButton.click();

    // Verify match started - wait for Match label or players to appear in header
    await expect(page.getByText('Match:', { exact: false }).filter({ visible: true }).first()).toBeVisible({ timeout: 20000 });
    
    // Check for ongoing status - use a more flexible search
    await expect(page.locator('div').filter({ hasText: /ongoing/i }).filter({ visible: true }).first()).toBeVisible({ timeout: 10000 });
  ...
  test('should allow starting a Chess 960 match', async ({ page }) => {
    ...
    // Toggle 960
    const chess960Button = page.getByRole('button', { name: 'CHESS 960' }).filter({ visible: true }).first();

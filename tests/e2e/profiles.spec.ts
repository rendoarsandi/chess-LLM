...
test.describe('ChessLLM Profiles', () => {
  test('should navigate from leaderboard to a player profile', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'PLAYER PROFILE' }).filter({ visible: true })).toBeVisible();
  ...
  test('should show all LLM profiles in the profiles page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'PROFILES' }).filter({ visible: true }).first()).toBeVisible();

...
export function initDb() {
  ...
  if (isTest) {
    ...
    try {
      ...
      // TEST INSERT
      try {
        _sqlite.prepare("INSERT INTO players (id, name, type, wins960, losses960, draws960, rating960, peak_rating960) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run('test-id', 'test-name', 'human', 0, 0, 0, 1200, 1200)
        ...
        try {
            ...
            for (const col of required) {
                if (!cols.includes(col)) {
                    ...
                    _sqlite.prepare(`ALTER TABLE players ADD COLUMN 
${col}
 INTEGER DEFAULT 0 NOT NULL`).run()
                    added = true
                }
            }
            if (added) {

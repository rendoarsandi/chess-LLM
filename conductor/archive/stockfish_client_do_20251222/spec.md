...
- [ ] The Client logs show the move being calculated by the _Player Worker_ (distinct from the Analysis Worker).
- [ ] The Server receives the move, updates the game state, and proceeds to the next turn.
- [ ] The UI Advantage Bar continues to update smoothly via the _Analysis Worker_ while the _Player Worker_ is thinking.
